/**
 * 프로젝트명 : 스마트창고관리 시스템
 * 프로그램명 : OrderDetailDialog.kt
 * 개 발 자 :  (주)디알젬
 * 업무기능 : 매입입고(발주입고) 시리얼번호 입력루틴
 */
        ...
        binding.btnAdd.setOnClickListener {
            ...
            if (baljuData.getJungyojajeyeobuHP() == "Y") {
                ...
                val serialData =  // 위에서 저장한 데이터를 다시 가져온다.
                    SerialManageUtil.getSerialStringByPummokCode("${baljuData.getPummokcodeHP()}")
                        .toString()
                // serialData와 contentString.toString()은 같은 데이터, 다시 읽을 필요는 없었다. (실제 확인함 by jung 2022.07.08)

                Log.d("yj 품목코드 = ", baljuData.getPummokcodeHP())
...
//        }
    ...
    override fun setValues() {
        ...
        beforeSuryang = baljuData.getPummokCount().toInt()
        ...
        if (baljuData.getPummokCount() != "0") { // 품목 수량이 0이 아니면 처리하는 것 모두 상세 분석할 것 2022.07.08 by jung
            ...
            if (baljuData.getJungyojajeyeobuHP() == "Y") { // 중요자재인 경우
        ...
        Log.d("yj", "data.pummokCount : ${baljuData.getPummokCount()}")
        ...
        binding.layoutCount      .isVisible = true            // 수량 text와 수량입력(edtCount) 칸이 보이도록한다.
        if (baljuData.getJungyojajeyeobuHP() != "Y") {        // 중요자재가 아닌 경우
