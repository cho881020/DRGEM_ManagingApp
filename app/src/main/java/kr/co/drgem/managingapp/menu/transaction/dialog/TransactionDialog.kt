/**
 * 프로젝트명 : 스마트창고관리 시스템
 * 프로그램명 : TransactionDialog.kt
 * 개 발 자 :  (주)디알젬
 * 업무기능 : 거매명세입고 시리얼번호 입력루틴
 */
...
import kr.co.drgem.managingapp.menu.kitting.dialog.DialogEditKittingAdapter
...
class TransactionDialog : BaseDialogFragment() {
    ...
    override fun setupEvents() {
        ...
        binding.btnAdd.setOnClickListener {
            ...
            if (!InputCountCheckChange(2,1)) {  //오류 검사    // by jung 20220701
            ...
            if (georaeData.getJungyojajeyeobuHP() == "Y") {
                mSqliteDB.deletePummokcodeSerials(georaeData.getPummokcodeHP()) // 2022.07.12 막음
                // 시리얼번호 편집
...
//        }
    ...
    override fun setValues() {
        ...
        beforeSuryang = georaeData.getPummokCount().toInt()
        ...
        if (georaeData.getPummokCount() != "0") { // 품목 수량이 0이 아니면 처리하는 것 모두 상세 분석할 것 2022.07.08 by jung
            ...
            if (georaeData.getJungyojajeyeobuHP() == "Y") { // 중요자재인 경우
        ...
        Log.d("yj", "data.pummokCount : ${georaeData.getPummokCount()}")

        // 2022.07.15일 입고는 품목코드 검사하지 않는 것으로 변경됨으로 화면에서 지운다.
        // 품목코드관련은 안보이게 한다. 아래 8줄 추가 됨, 종료시도 검사에서 생략
        binding.layoutCodeConfirm.isVisible = false
        binding.layoutCount      .isVisible = true            // 수량 text와 수량입력(edtCount) 칸이 보이도록한다.
        if (georaeData.getJungyojajeyeobuHP() != "Y") {        // 중요자재가 아닌 경우
            binding.btnAdd.isVisible = true                   // 등록하기 버튼이 보이고,
        }
        binding.edtCount.setText(georaeData.getPummokCount())  // 보관되어있던 수량을 표시한다.
        binding.edtCount.selectAll()
        binding.edtCount.requestFocus()
...
//        val mAdapter = DialogEditTranAdapter(viewholderCount, mSerialDataList)
        mAdapter = DialogEditTranAdapter(viewholderCount, mSerialDataList)
...
//        }
            ...
            val sss1 = binding.baljusuryang.text.trim().toString().toInt()    // 발주수량
            if (inputCount.toInt() > sss1) {
                AlertDialog.Builder(requireContext())
                    .setMessage("입력수량이 발주 수량보다 큽니다..")
                    .setNegativeButton("확인", null)
                    .show()
                if (sw02 == 1) {
                    binding.edtCount.selectAll()
